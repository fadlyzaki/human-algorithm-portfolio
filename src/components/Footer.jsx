...
import { ArrowUpRight, Heart, Home, Briefcase, Rocket, User, Palette, Mail, Linkedin, Instagram, Dribbble, Github } from 'lucide-react';
 ...
 */
const FooterTooltipLink = ({ to, href, children, tooltip, icon: Icon, external = false }) => {
    ...
    const linkClass = "relative text-sm hover:text-[var(--text-primary)] transition-colors duration-200 cursor-pointer";
    ...
    const tooltipEl = (
        ...
        >
            ...
            <span className="text-[11px] font-mono uppercase tracking-wider font-medium">{tooltip}</span>
        </span>
    );
            ...
            <div className="relative z-10">
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-8 mb-24 border-t border-[var(--text-secondary)]/20 pt-12">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <FooterTooltipLink to="/" icon={Home} tooltip="Start here">{t('nav.home')}</FooterTooltipLink>
                        <FooterTooltipLink to="/#work" icon={Briefcase} tooltip="Case studies & impact">{t('nav.work')}</FooterTooltipLink>
                        <FooterTooltipLink to="/side-projects" icon={Rocket} tooltip="Where ideas ship">{t('nav.side_projects')}</FooterTooltipLink>
                        <FooterTooltipLink to="/about" icon={User} tooltip="The full picture">{t('nav.about')}</FooterTooltipLink>
                        <FooterTooltipLink to="/design-system" icon={Palette} tooltip="Visual DNA">Design System</FooterTooltipLink>
                        <FooterTooltipLink to="/contact" icon={Mail} tooltip="Let's talk">{t('nav.contact')}</FooterTooltipLink>
                    </div>

                    {/* COL 2: SOCIALS */}
                    <div className="flex flex-col gap-4">
                        <h4 className="font-mono text-xs uppercase tracking-widest text-[var(--text-secondary)] mb-2">{t('footer.connect')}</h4>
                        <FooterTooltipLink href="https://www.linkedin.com/in/fadlyzaki/" external icon={Linkedin} tooltip="View profile">LinkedIn</FooterTooltipLink>
                        <FooterTooltipLink href="https://www.instagram.com/fadlyzaki" external icon={Instagram} tooltip="Behind the scenes">Instagram</FooterTooltipLink>
                        <FooterTooltipLink href="https://dribbble.com/fadlyzaki" external icon={Dribbble} tooltip="Visual explorations">Dribbble</FooterTooltipLink>
                        <FooterTooltipLink href="https://github.com/fadlyzaki/" external icon={Github} tooltip="Open source & code">GitHub</FooterTooltipLink>
                    </div>

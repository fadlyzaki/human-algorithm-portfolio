...
const ProfileScanner = ({
  ...
  return (
    ...
    >
      {/* CARD CONTAINER */}
      <div
        className={`relative w-full h-full rounded-lg overflow-hidden border border-[var(--border-color)] group-hover:border-[var(--accent-blue)] transition-colors duration-500 shadow-2xl bg-[var(--bg-card)]`}
      >
        {/* LAYERS CONTAINER */}
        <div className="absolute inset-0 z-0">
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-[var(--accent-blue)]/10 mix-blend-overlay transition-opacity duration-500 ${hasScanned || isHovered ? "opacity-0" : ""}`}
            ></div>
        ...
        <div className="absolute inset-0 z-30 pointer-events-none p-4 flex flex-col justify-between">
          ...
          <div
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ${isHovered ? "scale-100 opacity-100" : "scale-75 opacity-20"}`}
          >
            ...
            <Activity
              size={16}
              className="absolute inset-0 m-auto text-[var(--accent-red)] animate-pulse"
            />
          </div>
          ...
          <div className="flex justify-between items-end">
            ...
            <div className="font-mono text-[9px] text-[var(--text-secondary)] uppercase">
              Status:{" "}
              {isHovered ? (
                <span className="text-[var(--accent-green)] animate-pulse">
                  DETECTED
                </span>
              ) : (
                "SCANNING..."

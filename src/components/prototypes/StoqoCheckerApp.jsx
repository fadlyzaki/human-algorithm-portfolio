...
const StatusBar = ({ variant = "light" }) => {
  return (
    <div
      className={`h-11 ${variant === "light" ? "bg-[#ec5b13]" : "bg-[#ec5b13]"} flex justify-between items-center px-6 text-white text-xs font-semibold z-50 shrink-0 w-full`}
    >
      <span>12:30</span>
  ...
  return (
    <nav className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shrink-0">
      <div className="flex justify-around items-center h-16">
        <button
          onClick={() => onNavigate("/")}
          className={`flex flex-col items-center justify-center w-full h-full ${isTasks ? "text-[#ec5b13]" : "text-gray-500 dark:text-gray-400"}`}
        >
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-semibold text-[#ec5b13] uppercase tracking-wider">
              Active Task
            </span>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden border-l-4 border-[#ec5b13]">
            <div className="p-4">
              ...
              <div className="flex justify-between items-end mb-4">
                <div>
                  <p className="text-xs text-text-gray-500 dark:text-gray-400 mb-1">
                    Status Pengecekan
                  </p>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 bg-blue-50 text-blue-700 text-xs p-2 rounded border border-blue-100 flex items-start gap-2">
              ...
              <span>
                Driver sedang dalam perjalanan menuju lokasi Anda. Mohon
...
const CheckingDetailPage = ({ onNavigate }) => {
  ...
  const [items] = useState([
    {
      id: 1,
      name: "Cabe Merah Keriting (Ekonomis)",
      qty: "20 Pak",
      sub: "1 kg/Pak",
      status: "problem",
      icon: Package,
      ...
      <div className="flex-1 overflow-y-auto pb-24 no-scrollbar">
        <div className="bg-white dark:bg-gray-800 px-4 py-4 border-b border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex justify-between items-start mb-2">
            <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">
              Van H20 - TANGKOT 1 • F10R1
            </p>
            <span className="bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-wider">
              Belum Dicek
            </span>
          </div>
          <h2 className="text-base font-bold text-gray-900 dark:text-white mb-3">
            Warteg Jaya Bahari - Tatang
          </h2>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/10 px-4 py-3 border-b border-blue-100 dark:border-blue-800/30">
          <p className="text-[11px] leading-relaxed text-blue-800 dark:text-blue-200 flex items-center flex-wrap gap-1">
            Jika SKU sesuai, beri tanda centang{" "}
            <span className="inline-flex items-center justify-center bg-green-500 text-white rounded-full w-4 h-4">
              <Check size={8} />
            </span>
            . Bila tidak sesuai, beri tanda silang{" "}
            <span className="inline-flex items-center justify-center bg-red-500 text-white rounded-full w-4 h-4">
              <X size={8} />
            </span>{" "}
            dan pilih keterangan.
          </p>
        </div>
        ...
        <div className="p-4 space-y-4">
          {items.map((item) => (
            <div
              key={item.id}
              className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-200 ${item.status === "checked" ? "opacity-60" : ""}`}
            >
              <div className="p-3 flex items-start space-x-3">
                <div
                  className={`w-16 h-16 rounded ${item.bgColor} shrink-0 overflow-hidden relative flex items-center justify-center`}
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex space-x-2">
                      {item.status === "checked" ? (
                        ...
                        <>
                          <button
                            className={`w-8 h-8 rounded-full ${item.status === "problem" ? "bg-red-500" : "bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600"} flex items-center justify-center shadow-sm hover:opacity-90 transition`}
                          >
                            <X
                              className={`${item.status === "problem" ? "text-white" : "text-gray-400"} text-lg`}
                              size={18}
                            />
                          </button>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <input
                          className="w-10 text-center text-xs font-semibold border-none bg-transparent text-gray-900 dark:text-white p-0 focus:ring-0"
                          type="text"
                          defaultValue="3"
                        />
                        <button className="w-7 h-7 bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded flex items-center justify-center hover:bg-blue-200 dark:hover:bg-blue-900/60 transition font-bold text-lg">

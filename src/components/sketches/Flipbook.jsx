...
export const Flipbook = ({ pages, initialPage = 0 }) => {
  ...
  const [spreadIndex, setSpreadIndex] = useState(
    initialPage > 0 ? Math.ceil(initialPage / 2) : 0,
  );
  ...
  const renderPageContent = (pageIndex) => {
    ...
    if (pageIndex === -1) {
      ...
      if (bookMedium === "digital") {
        return (
          <div
            className={`w-full h-full flex flex-col items-center justify-center p-8 shadow-inner bg-zinc-950 border-zinc-900 border-r-0`}
          >
    ...
    if (pageIndex === paddedPages.length) {
      ...
      if (bookMedium === "digital") {
        return (
          <div
            className={`w-full h-full flex items-center justify-center p-8 shadow-inner bg-zinc-950 border-zinc-900`}
          >
            <div className="absolute inset-0 z-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20"></div>
            <div className="w-[80%] h-[90%] flex flex-col items-center justify-center text-center relative z-10">
              <div className="w-16 h-16 rounded-sm border border-blue-500/20 flex items-center justify-center bg-black/40 rotate-45">
                <span className="text-blue-500/30 font-mono text-xl -rotate-45 font-bold">
                  FZ
                </span>
              </div>
            </div>
    ...
    return (
      ...
      >
        {/* Paper Texture */}
        <div
          className={`absolute inset-0 z-0 pointer-events-none ${isDark ? "opacity-5" : "opacity-40"} mix-blend-multiply`}
          style={{
        ...
        <div className="mt-8 relative z-20 flex flex-col items-center text-center w-full">
          ...
          <div
            className={`w-8 h-[1px] mb-2 ${isDark ? "bg-white/20" : "bg-black/10"}`}
          ></div>
          <span
            className={`font-mono text-[9px] uppercase tracking-widest ${isDark ? "text-white/40" : "text-zinc-400"}`}
          >
            {pageIndex + 1}
          </span>
        </div>
  ...
  const bookStyleTransform = isMobile
    ? "none"
    : isCover

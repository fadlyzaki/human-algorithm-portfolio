...
const DesignSystemCard = ({ expanded, showChrome, isId }) => (
  <Container
    expanded={expanded}
    showChrome={showChrome}
    figIndex="9.1"
    schematicType={isId ? "PUSTAKA_KOMPONEN_ATOMIK" : "ATOMIC_COMPONENT_LIBRARY"}
  >
    <div className="w-full h-full p-12 flex items-center justify-center">
      <div className="grid grid-cols-2 gap-4 w-full max-w-[200px]">
        ...
        <div className="col-span-2 p-3 border border-[var(--brand)]/20 rounded-lg flex flex-col gap-2 bg-[var(--brand)]/5 animate-[pulse_4s_ease-in-out_infinite]">
          <span className="text-[6px] font-mono text-[var(--brand)] uppercase">
            {isId ? "MOL_KARTU" : "MOL_CARD"}
          </span>
        </div>
      </div>
    </div>

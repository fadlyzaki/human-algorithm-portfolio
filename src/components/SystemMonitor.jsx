  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg p-6 font-mono text-xs shadow-xl backdrop-blur-sm">
      <div className="flex justify-between items-center mb-6 border-b border-[var(--border-color)] pb-2">
        ...
        <div className="text-[var(--text-card-secondary)]">UPTIME: 99.9%</div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {skills.map((skill, idx) => (
          <div key={idx} className="space-y-2 group">
            ...
            {skill.url && (
              <div className="absolute right-0 top-0 w-2 h-2 bg-[var(--accent-blue)] rounded-full animate-pulse opacity-50"></div>
            )}
            ...
            <div className="text-[9px] text-[var(--text-card-secondary)] opacity-50 uppercase tracking-wider flex justify-between items-center">
              <div className="flex gap-3">
                ...
                <span className="hidden sm:inline">
                  MEM: {metrics[skill.name] * 12}MB
                </span>
                <span className="sm:hidden">
                  M: {metrics[skill.name] * 12}MB
                </span>
              </div>

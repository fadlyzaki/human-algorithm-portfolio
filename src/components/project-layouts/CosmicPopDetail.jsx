...
const CosmicPopDetail = ({ project, activeContext, activeChallenge, activeProcess, activeInsights, activeSolution, activeMetrics, activeLearnings, InteractionComponent, showLivePreview, setShowLivePreview, t, isIndonesian, activeTitle, activeTldr, activeSnapshot }) => {
    ...
    return (
        <div className="bg-[#10101A] text-[#E2E8F0] font-sans min-h-screen selection:bg-[#00C2FF]/30 overflow-hidden relative">
            ...
            <main className="relative z-10 pt-24 pb-32 max-w-6xl mx-auto px-6">
                ...
                <header className="py-20 flex flex-col items-center text-center space-y-10">
                    ...
                    <motion.div
                        ...
                        className="w-24 h-24 rounded-[2rem] bg-gradient-to-tr from-[#00C2FF]/20 to-[#6B46C1]/20 border border-white/10 flex items-center justify-center backdrop-blur-xl shadow-[0_0_50px_rgba(0,194,255,0.2)]"
                    >
                        <Activity size={40} className="text-[#00C2FF]" />
                    </motion.div>

                    <nav className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full backdrop-blur-md">
                    ...
                    <h1 className="text-6xl md:text-8xl font-black italic tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-white via-blue-100 to-[#00C2FF] max-w-4xl leading-[1.1]">
                        {activeTitle}
                    </h1>

                    <p className="text-xl md:text-2xl text-blue-200/70 font-medium max-w-2xl leading-relaxed">
                        {activeTldr}
                    </p>

                    <div className="flex flex-wrap justify-center gap-4 pt-8">
                        {project.links.demo && project.links.demo !== '#' && (
                            <a href={project.links.demo} className="flex items-center gap-2 bg-[#00C2FF] text-[#10101A] px-8 py-4 rounded-full font-bold uppercase tracking-widest hover:scale-105 hover:shadow-[0_0_30px_rgba(0,194,255,0.4)] transition-all">
                                <Play size={16} fill="currentColor" /> Play Demo
                            </a>
                        )}
                        <div className="flex bg-white/5 backdrop-blur-xl border border-white/10 rounded-full p-1 h-14">
                            {project.stack.slice(0, 3).map(tech => (
                                <span key={tech} className="px-6 py-3 text-xs font-bold uppercase tracking-wider text-blue-200">
                                    {tech}
                                </span>
                            ))}
                        </div>
                ...
                <section className="mt-32 space-y-32">
                    ...
                    {/* The Challenge */}
                    <article className="bg-gradient-to-br from-white/5 to-transparent border border-white/10 rounded-[3rem] p-12 md:p-16 relative overflow-hidden group">
                        ...
                        <h2 className="text-3xl md:text-4xl font-bold leading-relaxed text-white max-w-3xl">
                            "{activeChallenge}"
                        </h2>
                    </article>
                    ...
                    {activeProcess && (
                        <div className="grid md:grid-cols-2 gap-8">
                            {activeProcess.map((step, idx) => (
                                <article key={idx} className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden flex flex-col hover:-translate-y-2 transition-transform duration-500">
                                    {step.image && (
                                    ...
                                    <div className="p-8 flex-1 flex flex-col">
                                        ...
                                        <p className="text-blue-200/60 leading-relaxed font-medium">
                                            {step.desc}
                                        </p>
                                    </div>
                                </article>
                    ...
                    {activeInsights && activeInsights.length > 0 && (
                        <div className="grid md:grid-cols-2 gap-8">
                            {activeInsights.map((insight, idx) => (
                                <div key={idx} className="group p-8 rounded-3xl bg-blue-900/10 border border-blue-500/20 hover:bg-blue-500/10 transition-colors">
                                    <div className="mb-6 w-12 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center text-[#00C2FF]">
                                        <Star size={24} />
                                    </div>
                                    <h3 className="text-xl font-bold mb-3 text-white">{insight.title}</h3>
                                    <p className="text-blue-200/70 leading-relaxed font-medium">{insight.desc}</p>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {(activeMetrics || activeLearnings) && (
                        <div className="bg-gradient-to-r from-[#6B46C1]/20 to-[#00C2FF]/20 rounded-[3rem] p-12 md:p-16 border border-white/10 flex flex-col md:flex-row items-center gap-16">
                            {activeLearnings && (
                            ...
                            {activeMetrics && (
                                <div className="flex gap-8 border-l border-white/10 pl-16">
                                    {activeMetrics.map((m, i) => (
                                        <div key={i} className="text-center">
                                            <div className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-[#00C2FF] mb-2">{m.value}</div>
                                            <div className="text-xs font-bold uppercase tracking-widest text-blue-200/50">{m.label}</div>
                                        </div>
                                    ))}
                                </div>
                            )}

...
const BrandStrategy = ({ isXRayMode }) => {
    const renderNode = (title, items) => (
        <div className="p-6 border border-[var(--border-color)] bg-[var(--bg-card)] hover:border-[var(--accent)] transition-all duration-300 group relative overflow-hidden flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 mb-6 border-b border-[var(--border-color)] pb-4 relative z-10">
                ...
                <h4 className="font-mono text-xs uppercase tracking-widest text-[var(--text-primary)] group-hover:text-[var(--accent)] transition-colors">{title}</h4>
            </div>

            <ul className="space-y-3 relative z-10 flex-grow flex flex-col justify-end">
                {items.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                        <span className="font-mono text-[10px] text-[var(--text-secondary)] mt-0.5 opacity-40 group-hover:text-[var(--accent)] group-hover:opacity-100 transition-colors">0{idx + 1}</span>
                        <span className="text-xs text-[var(--text-secondary)] tracking-wide group-hover:text-[var(--text-primary)] transition-colors leading-relaxed">{item}</span>
                    </li>
                ))}
            </ul>
        </div>
    ...
    return (
        <div className={`space-y-16 animate-in slide-in-from-bottom-8 duration-500 py-8 relative ${isXRayMode ? 'p-4 border border-dashed border-[var(--accent)]/50 bg-[var(--accent)]/5' : ''}`}>
            {isXRayMode && <span className="absolute -top-3 left-2 z-20 bg-[var(--bg-void)] px-1 font-mono text-[8px] text-[var(--accent)]">BrandStrategy.jsx // DataMatrix</span>}

            {/* Header Area */}
            <div className={`text-center space-y-4 max-w-2xl mx-auto relative ${isXRayMode ? 'p-4 border border-dashed border-[var(--text-secondary)]/50' : ''}`}>
                {isXRayMode && <span className="absolute -top-3 left-2 z-20 bg-[var(--bg-void)] px-1 font-mono text-[8px] text-[var(--text-secondary)]">Section // Header</span>}
                <span className="font-mono text-[10px] md:text-xs uppercase tracking-[0.3em] text-[var(--text-secondary)]">System Architect Profile</span>
                <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-[var(--text-primary)]">Strategic Core</h2>
                <div className="h-1 w-12 bg-[var(--accent)] mx-auto mt-6"></div>
            </div>

            {/* BRAND FOUNDATION */}
            <div className={`space-y-8 relative z-10 ${isXRayMode ? 'p-4 border border-dashed border-[var(--text-secondary)]/50' : ''}`}>
                {isXRayMode && <span className="absolute -top-3 left-2 z-20 bg-[var(--bg-void)] px-1 font-mono text-[8px] text-[var(--text-secondary)]">Section // Foundation Grid</span>}
            ...
            {/* BRAND ACTIVATION */}
            <div className={`space-y-8 relative z-10 ${isXRayMode ? 'p-4 border border-dashed border-[var(--text-secondary)]/50' : ''}`}>
                {isXRayMode && <span className="absolute -top-3 left-2 z-20 bg-[var(--bg-void)] px-1 font-mono text-[8px] text-[var(--text-secondary)]">Section // Execution Grid</span>}

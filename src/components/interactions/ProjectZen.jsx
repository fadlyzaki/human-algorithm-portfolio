...
export default function NetflixContextUI() {
  ...
  const filteredContent = contentDatabase.filter((item) => {
    ...
    if (activeFilter === "focus")
      return item.attention === "High" || item.attention === "Max";
    return true;
  });
      ...
      <div className="relative h-[50vh] md:h-[60vh] w-full bg-zinc-900 flex items-end pb-12 px-4 md:px-12">
        ...
        <div className="relative z-20 max-w-2xl">
          ...
          <p className="text-lg text-gray-200 mb-6 drop-shadow-md">
            Stop scrolling. Start watching. Filter your library based on your
        ...
        <div className="mb-2 flex items-center gap-2 text-gray-400 text-sm font-semibold uppercase tracking-widest">
          ...
          {isCalmMode && (
            <span className="text-xs normal-case bg-green-900/50 text-green-400 px-2 py-0.5 rounded border border-green-800 flex items-center gap-1">
              <VolumeX size={10} /> Calm Mode Active
            </span>
          )}
        </div>
...
function FilterButton({ icon, label, sub, isActive, onClick }) {
  return (
    ...
    >
      ...
      {sub && (
        <span
          className={`text-[11px] font-medium mt-0.5 block ${isActive ? "text-red-100 opacity-90" : "text-gray-600"}`}
        >
          {sub}
        </span>
      )}
...
function ContentCard({ item, isHovered, setHovered, isCalmMode }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`
        relative bg-[var(--bg-card)] rounded md:rounded-md overflow-hidden transition-all duration-300 origin-center
          ...
          <div className="p-3">
            ...
            <div className="mt-3 mb-2 bg-[var(--bg-card)] p-2 rounded border border-gray-800">
              <div className="flex justify-between items-center mb-1">
                ...
                <span
                  className={`text-[10px] font-bold ${getAttentionColor(item.attention, "text")}`}
                >
                  {item.attention === "Low"
                    ? "Chill"
                    : item.attention === "Med"
              ...
              <div className="w-full bg-gray-700 h-1.5 rounded-full overflow-hidden">
                <div
                  className={`h-full ${getAttentionColor(item.attention, "bg")} transition-all duration-300`}
                  style={{ width: getAttentionWidth(item.attention) }}
                ></div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {item.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] text-gray-300 flex items-center"
                >

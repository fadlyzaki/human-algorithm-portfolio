    ...
    return (
        <div className="flex justify-center font-sans antialiased rounded-xl overflow-hidden">
            <div className="w-full max-w-sm bg-white h-[600px] relative shadow-[0_0_40px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden">
                {screen === 'search' && <SearchScreen flight={flight} setScreen={setScreen} />}
                {screen === 'search' && <BottomNav screen={screen} setScreen={setScreen} />}
                {screen === 'lock-modal' && (
                    <>
                        <SearchScreen flight={flight} setScreen={setScreen} />
                        <LockModal flight={flight} setScreen={setScreen} handleLock={handleLock} />
                    </>
                )}
                {screen === 'success' && <SuccessScreen timeLeft={timeLeft} formatTime={formatTime} setScreen={setScreen} />}
                {screen === 'bookings' && <BookingsScreen lockedItem={lockedItem} timeLeft={timeLeft} formatTime={formatTime} setScreen={setScreen} />}
            </div>
        </div>
    );
};

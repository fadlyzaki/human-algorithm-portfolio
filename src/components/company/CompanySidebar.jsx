      ...
      <div className="sticky top-32 space-y-12">
        ...
        <div className="p-6 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg">
          <div className="text-[var(--brand)] mb-4">
            <IconMapper
              iconName={cluster.companyFocus?.icon || "Cpu"}
              size={24}
            />
          </div>
          <h4 className="font-serif italic text-lg mb-2">
            {isId
              ? cluster.companyFocus_id?.title ||
                cluster.companyFocus?.title ||
                t("company.tech_core")
              : cluster.companyFocus?.title || t("company.tech_core")}
          </h4>
          <ul className="space-y-2">
            {(isId
              ? cluster.companyFocus_id?.items ||
                cluster.companyFocus?.items || [
                  "React Ecosystem",
                  "Node.js",
                  "Python Data Science",
                ]
              ...
              >
                <span className="w-1 h-1 bg-[var(--text-primary)] rounded-full"></span>{" "}
                {t}
              </li>
            ))}
          </ul>
        ...
        <div className="relative p-6 pt-10 border border-[var(--border-color)] rounded-lg bg-[var(--bg-card)] overflow-hidden group">
          ...
          <blockquote className="relative">
            <span className="absolute -top-4 -left-2 text-4xl text-[var(--brand)] opacity-20 font-serif">
              "
            </span>
            <p className="text-sm font-serif italic leading-relaxed text-[var(--text-primary)] relative z-10">
              {isId
                ? cluster.motivation_id || cluster.motivation
                : cluster.motivation}
            </p>
            <footer className="mt-4 flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-[8px] font-mono text-[var(--text-secondary)] opacity-60">
                  {(() => {
                    const roleStat = cluster.stats?.find(
                      (s) => s.label === "Role",
                    );
                    const roleValue = roleStat
                      ? roleStat.value
                      : "Product Designer";

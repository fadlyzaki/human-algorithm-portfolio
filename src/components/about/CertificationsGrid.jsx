      ...
      <ScrollReveal>
        <div className="flex items-baseline gap-4 mb-12 border-b border-[var(--border-color)] pb-4">
          ...
          <h2 className="text-2xl font-mono text-[var(--text-primary)] uppercase tracking-tight">
            {t("about.section_knowledge_upgrades")}
          </h2>
        </div>
                  ...
                  {cert.skills && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {cert.skills.map((skill) => (
                        <span
                          key={skill}
                          className="text-[9px] font-mono text-[var(--text-card-secondary)] py-0.5 px-2 rounded-md bg-[var(--bg-void)]/30 border border-[var(--border-color)]/20 whitespace-nowrap"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="mt-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-4 border-t border-[var(--border-color)]/10">
                    {cert.id && cert.id !== "N/A" && (
                      <div className="flex md:flex-col gap-2 md:gap-0.5 items-center md:items-start">
                        <span className="font-mono text-[8px] text-[var(--text-card-secondary)] opacity-40 uppercase">
                          Certificate_ID
                        </span>

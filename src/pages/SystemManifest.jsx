    ...
    >
      ...
      <div className="max-w-[210mm] mx-auto bg-[var(--bg-void)] md:bg-[var(--bg-panel)] md:p-12 md:shadow-2xl md:border border-[var(--border-color)] print:border-none print:shadow-none print:p-0 relative z-10">
        ...
        <header className="border-b-2 border-[var(--text-primary)] pb-6 mb-8">
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-mono uppercase tracking-tight mb-2">
              {header.name}
            </h1>
            <h2 className="text-lg text-[var(--text-secondary)] font-mono mb-3">
              {header.role}
            </h2>
          </div>
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-mono text-lg uppercase font-bold border-b border-[var(--border-color)] w-full pb-1 print:border-black">
            ...
            <div className="space-y-10">
              {experience.map((job, idx) => (
                <div key={idx} className="group">
                  ...
                  <div className="text-[var(--accent-mono)] font-mono text-sm mb-3">
                    {job.role}
                  </div>
                  <p className="text-[var(--text-secondary)] italic mb-4 text-sm">
                    "{job.summary}"
                  </p>
                  ...
                  <ul className="space-y-2">
                    {job.metrics.map((metric, mIdx) => (
                      ...
                      >
                        <CheckCircle
                          size={14}
                          className="mt-1 shrink-0 text-[var(--text-secondary)] opacity-50"
                        />
                        <span>{metric}</span>
                      </li>
                    ))}
          ...
          <aside className="space-y-10">
            ...
            <section>
              ...
              <div className="space-y-6">
                <div>
                  <h5 className="font-mono text-xs text-[var(--text-secondary)] uppercase mb-2 print:text-black print:font-bold">
                  ...
                  <div className="flex flex-wrap gap-2 print:gap-0">
                    {[
                      ...
                      <span
                        key={s}
                        className="bg-[var(--border-color)] px-2 py-1 text-xs font-mono rounded-sm print:bg-transparent print:px-0 print:py-0 print:text-black"
                      >
                        {s}
                        {i < arr.length - 1 ? (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 print:gap-0">
                    {[
                      ...
                      <span
                        key={s}
                        className="border border-[var(--border-color)] px-2 py-1 text-xs font-mono rounded-sm text-[var(--text-secondary)] print:border-none print:px-0 print:py-0 print:text-black"
                      >
                        {s}
                        {i < arr.length - 1 ? (
                          <span className="hidden print:inline">, </span>
            ...
            <section>
              ...
              {education.map((edu, idx) => (
                <div key={idx} className="mb-4">
                  ...
                  <div className="font-mono text-xs text-[var(--accent-mono)] mt-1">
                    {edu.year}
                  </div>
                </div>
              ))}

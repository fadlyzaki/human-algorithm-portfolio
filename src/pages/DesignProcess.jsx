          ...
          <ScrollReveal>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-start">
              <p className="text-xl text-[var(--text-secondary)] leading-relaxed font-light">
                Design is not just visual polish; it is the rigorous
                architecture of value. My process is a recursive loop of
                deconstruction, synthesis, interaction, and validation—
          ...
          {processSteps.map((step, index) => (
            <ScrollReveal key={step.id} delay={index * 100}>
              <div className="group relative bg-[var(--bg-surface)] border border-[var(--border-color)] hover:border-[var(--text-primary)] transition-all duration-500 rounded-2xl overflow-hidden">
                {/* Hover Gradient */}
                <div
                  className={`absolute top-0 left-0 w-1 h-full ${step.bg} transition-all duration-500 group-hover:w-2`}
                ></div>
                ...
                <div className="flex flex-col lg:flex-row">
                  ...
                  <div className="lg:w-1/3 p-8 border-b lg:border-b-0 lg:border-r border-[var(--border-color)] flex flex-col justify-between relative overflow-hidden">
                    <div className="relative z-10">
                      <span
                        className={`font-mono text-xs font-bold ${step.color} mb-4 block`}
                      >
                        PHASE_{step.id}
                      </span>
                    ...
                    <div className="p-8 border-b md:border-b-0 md:border-r border-[var(--border-color)]">
                      ...
                      <ul className="space-y-3">
                        {step.inputs.map((item, i) => (
                          <li
                            key={i}
                            className="flex items-start gap-3 text-sm text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors"
                          >
                    ...
                    <div className="p-8 bg-[var(--bg-card)]/50">
                      ...
                      <ul className="space-y-3">
                        {step.outputs.map((item, i) => (
                          <li
                            key={i}
                            className="flex items-start gap-3 text-sm text-[var(--text-primary)] font-medium"
                          >
                            <ArrowRight
                              size={14}
                              className={`mt-0.5 ${step.color} opacity-80`}
                            />
                            {item}
                          </li>
                        ))}
                      ...
                      <div className="mt-8 flex flex-wrap gap-2">
                        {step.tools.map((tool, tIdx) => (
                          <span
                            key={tIdx}
                            className="px-2 py-1 text-[10px] font-mono border border-[var(--border-color)] rounded text-[var(--text-secondary)] bg-[var(--bg-void)]"
                          >
                            {tool}
                          </span>
                        ))}
